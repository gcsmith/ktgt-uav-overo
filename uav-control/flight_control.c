...
void *takeoff_land(void *fd)
{
    ...
    else
    {
        ...
        while (!stable)
        {
            pthread_mutex_lock(&m_data->gpio_alt->lock);
            while ((input = (m_data->gpio_alt->pulsewidth / 147)) != pid.setpoint)
            {
                pthread_mutex_unlock(&m_data->gpio_alt->lock);
